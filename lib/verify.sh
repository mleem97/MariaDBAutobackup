...
calculate_checksum() {
    local backup_dir=$1
    echo "Calculating checksums for backup files in $backup_dir..." | tee -a "${LOG_FILE:-$LOG_DIR/mdbackup.log}"
    find "$backup_dir" -type f -name "*.sql*" | while read -r file; do
        if command -v sha256sum &> /dev/null; then
            ...
            echo "Warning: No checksum tool found. Skipping checksum calculation." | tee -a "${LOG_FILE:-$LOG_DIR/mdbackup.log}"
            return
        fi
    done
    echo "Checksums calculated and stored in $backup_dir/checksums.sha256" | tee -a "${LOG_FILE:-$LOG_DIR/mdbackup.log}"
}
...
verify_backup_integrity() {
    ...
    echo "Verifying backup integrity for $BACKUP_PATH..." | tee -a "${LOG_FILE:-$LOG_DIR/mdbackup.log}"
    if command -v sha256sum &> /dev/null; then
    ...
    if [ $? -eq 0 ]; then
        echo "Backup integrity verified successfully!" | tee -a "${LOG_FILE:-$LOG_DIR/mdbackup.log}"
    else
