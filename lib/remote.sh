...
transfer_backup_to_remote() {
    if [ -z "$REMOTE_BACKUP_ENABLED" ] || [ "$REMOTE_BACKUP_ENABLED" != "yes" ]; then
        echo "Remote backup is disabled. Skipping transfer." | tee -a "${LOG_FILE:-$LOG_DIR/mdbackup.log}"
        return
    fi

    echo "Transferring backup to remote storage..." | tee -a "${LOG_FILE:-$LOG_DIR/mdbackup.log}"
    ...
    if [ -z "$REMOTE_NFS_MOUNT" ] && [ -z "$REMOTE_RSYNC_TARGET" ] && 
       [ -z "$REMOTE_CLOUD_CLI" ] || [ -z "$REMOTE_CLOUD_BUCKET" ]; then
        echo "Warning: Remote backup is enabled but no valid method is configured." | tee -a "${LOG_FILE:-$LOG_DIR/mdbackup.log}"
        return
    fi

    if [ -n "$REMOTE_NFS_MOUNT" ]; then
        echo "Using NFS mount: $REMOTE_NFS_MOUNT" | tee -a "${LOG_FILE:-$LOG_DIR/mdbackup.log}"
        ...
        echo "Using rsync target: $REMOTE_RSYNC_TARGET" | tee -a "${LOG_FILE:-$LOG_DIR/mdbackup.log}"
        ...
        echo "Using cloud storage: $REMOTE_CLOUD_BUCKET" | tee -a "${LOG_FILE:-$LOG_DIR/mdbackup.log}"
    ...
    echo "Backup successfully transferred to remote storage." | tee -a "${LOG_FILE:-$LOG_DIR/mdbackup.log}"
}